"""
This code consumes services from some cyber threat providers, the idea of this tool is to provide utilities to make decisions about threats, 
for example, a malicious communication in a network
+ API from Virustotal
+ Ransomware Tracker
"""
import requests
...
def parser_menu():
    ...
    parser = optparse.OptionParser(
        "Options to do -t <tool> -u <url> -a <api key> -o <host>")
...
def menu_ransomware_tracker(hosts=["www.google.com"]):
    ...
    ransomware_df = pd.read_csv(
        url_ramsomware, skiprows=8, encoding="ISO-8859-1")
    ransomware_df.columns = ransomware_df.columns.str.replace(
        "-", "_").str.replace("(", "_").str.replace(")", "_").str.lower().str.replace(" ", "_")
    ransomware_df = ransomware_df.loc[ransomware_df.ip_address_es_.isin(
        hosts)].copy()
